...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; --선동일D9 송종이D9 노옹철D9

--3) 부서코드가 노옹철사원과 같은 소속의 직원 명단 조회
--1)과 2) 에 작성한 쿼릐를 하나의 쿼리로 합쳐주기
SELECT EMP_NAME DEPT_CODE
FROM EMPLOYEE
...
서브쿼리
EMPLOYE 테이블에서 전체 직원의 평균 급여를 계산
JOB 테이블의 각 행에 대해 평균급여가 계산된 서브쿼릭 실행되기 때문에
각 직급에 대해 동일한 평균 급여값을 반환

위 코드의 문제점 : 각 직급별 개별적인 평균 급여는 반환하지 않음
만약에 각 직급별로 평균 급여를 정확하게 계산하고 개별적인 평균급여 반환하길 원한다면
서브쿼리에 WHERE E.JOB_CHODE = J.JOB;CODE를 넣어줘야함
*/
