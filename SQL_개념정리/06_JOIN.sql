...
-관계형 데이터베이스에서 SQL 이용해서 테이블간 관계를 맺는 방법
...
직원ID, 이름, 부서코드, 부서ID, 부서명을 조회는 쿼리

JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);

EMP_ID    EMP NAME   DEPT CODE  DEPT ID    DEPT TITLE
200	        선동일	    D9	        D9	     총무부
201	        송종기	    D9	        D9	     총무부
202	        노옹철	    D9	        D9	     총무부
...
-- ANSI방식 JOIN ON LOCATION_ID = LOCAL_CODE
SELECT DEPT_TITLE AS "부서명", LOCAL_NAME AS "지역명" , LOCAL_CODE , LOCATION_ID
FROM DEPARTMENT 
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
-- ORACLE 방식 JOIN ON LOCATION_ID = LOCAL_CODE
...
-- 2) 연결에 사용할 두 컬럼이 다른 경우 > JOIN USING을 사용
FROM 테이블명
...
테이블을 다수 여러개 참조할 대 유용

사용방법

1) SELECT 테이블명,컬럼명,테이블명,컬럼명
    FROM  테이블명,테이블명
    WHERE 테이블명,칼럼명 = 테이블명,컬럼명,
...
-- 1) LEFT [OUTER] JOIN
-- 합치기에 사용한 두 테이블 중에서 왼족 편에 작성된 테이블의
-- 컬럼수를 기준으로 JOIN
