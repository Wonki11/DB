...
오라클에서 객체 : 테이블(TABLE) 뷰(VIEW) 시퀀스(SEQUENCE)
              인데그(INDEX) 패키지(PACKAGE) 트리거(TRIGGER)
...
/*
QL 오류: ORA-01400: cannot insert NULL into
NULL값을 넣을 수 없음 
...
CREATE TABLE UNIQUE_TABLE (
    ...
    -- (방법 1)    
    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- CONSTRAINT(중복X) 제약조건명 제약조건 종류 
                                                      -- 컬럼 레벨 제약 조건 (이름 O)
    -- (방법 2) 
...
CREATE TABLE KH_CAFE ( 
    ...
    -- VARCHAR2() 숫자가 초과해 오류가 날시 ALTER를 사용해 컬럼 크기를 수정할 수 있음
...
CREATE TABLE USER_PRIMARY_TABLE ( 
 ...
 -- (방법 3)
    USER_NO NUMBER(10,0) 
    CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO)>연결할컬럼명
*/
CREATE TABLE USER_PRIMARY_TABLE ( 
...
--UNIQUE는 칼럼에서 NULL이 한 개는 허용됨 두 개 이상 안됨
...
-- parent key not found 부모키인 USER_GRADE 에 40이라는 값이 없다 INSERT INTO 에 40이 구문이 없다

INSERT INTO USER_GRADE VALUES (40,'플래티넘회원');

INSERT INTO USER_GRADE VALUES (00, '임시회원'); -- 정수에서 맨 앞의 0은 사라진다 
-- 핸드폰번호를 기입할 때는 VARCHAR2를 사용해서 010 모두 들어갈 수 있도록 설정
SELECT * FROM USER_GRADE; 

--만약에 USER01 이라는 회원 일반회원으로 지정이 되어 있는데
...
--배달 어플 A카페 폐업 
-- 배달 어플 > A카페 카페마음대로 폐업 못함
-- A카페가 열어놓은 메뉴들은 모두 삭제 
-- A카페 폐업할 때 연결된 메뉴까지 한 번에 삭제 

--CASCADEE -- 부모키 삭제시 참조하는 자식의 행도 모두 삭제 

--삭제는  아니고 메뉴들은 NULL값으로 변경 
-- NULL 적용
...
-- USER_GRADE GRADE_CODE 참조하는 테이블 생성
CREATE TABLE 자식테이블 ( 
컬럼1 자료형 NUMBER(10,0) PRIMARY KEY, 
컬럼2 자료형 VARCHAR2(20) UNIQUE, 
컬럼3 자료형 VARCHAR2(30) NOT NULL, 
...
(방법3) - 부모키 삭제시 자식키도 함게 삭제 : ON DELETE CASCADE  **&&중요&&**
...
CREATE TABLE USER_CHECK(USER_NO NUMBER PRIMARY KEY, 
USER_ID VARCHAR2(20) UNIQUE, 
USER_PW VARCHAR2(30) NOT NULL ,
...
--INSERT INTO USER_CHECK VALUES(2,'USER02','PW02','박철수','남자') '남'만 가능 
