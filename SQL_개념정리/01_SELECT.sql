...
--컬럼순서를 이용해서 정렬하는 방법
--EMPLOYEE 테이블에서 급여가 300 이상 600이하인 사원의 사번 이름 급여를 이름 내림차순으로 조회
...
FROM EMPLOYEE 
WHERE SALARY BETWEEN 3000000 AND 6000000
ORDER BY EMP_NAME 1 DESC; -- 1 = EMP_NAME , 
...
SELECT EMP_NAME,SALARY * 12 AS "연봉"
FROM EMPLOYEE
ORDER BY SALARY DESC; -- SALARY * 12 대신 "연봉" 작성 가능

-- 주의할 점 : ORDER BY에서는 별칭 사용이 가능하지만
-- WHERE 절의 경우 조건이기 때문에 별칭 사용이 불가능
-- ORDER BY는 결과를 가지고 결과 정리를 하는 표현방법
-- WHERE 은 결과를 나타내기 위해 찾는 조건문이기 때문 , 조건이 뭔가 진행도 안됐는데 별칭부터 붙인 것이기 때문
...
D1 부서의 경우 내림차순 이기 때문에 J7이 J6보다 먼저 정렬
...
FROM EMPLOYEE 
ORDER BY 부서코드 ASC,직급코드 DESC, 이름 ASC; --중첩

--6. EMPLOYEE 테이블에서 모든 직원의 이메일을 이메일 표기 오름차순 정렬 
--   전화번호 AS 전화번호 EMAIL내에서는 전화번호 내림차순 정렬
SELECT EMP_NAME,EMAIL AS "이메일",PHONE AS "전화번호" FROM EMPLOYEE
ORDER BY 이메일 ASC, 전화번호 DESC;

--7. EMPLOYEE 테이블에서 모든 직원의 이름 "직원명" 표기
-- 직원명 내림차순 입사일 "입사일자" 표기 같은 이름 내에서 오름차순으로 정렬 조회
