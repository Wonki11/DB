...
- 순차적으로 일정한 간격의 숫자(번호)를 발생시키는 객체 
    ...
    EX) 1번째 회원, 2번째 회원,3번째 회원 ... N번 째 회원
    
[작성법]
CREATE SEQUENCE 시퀀스이름
[STRAT WITH 숫자] -- 처음 발생시킬 시작값 지정, 생략하면 자동으로 1이 기본값으로 설정 
...
[CACHE 바이트크기 | NOCACHE] -- 캐쉬메모리 기본값은 20BYTE 최소값은 2BYTE 
    ...
    -- 시퀀스 호출 시 미리 저장되어진 값을 가져와 반환하므로 매번 시퀀스를 생성해서 반환하는 것 보다 DB속도 향상

* 사용 방법
시퀀스명.NEXTVAL : 다음 시퀀스 번호 얻어옴

시퀀스명.CURRAVL : 현재 시퀀스 번호를 얻어옴
...
CREATE TABLE TB_TEST (
 ...
 CREATE SEQUENCE SEQ_TEST_NO; -- 시퀀스는 괄호없이 세미콜론 
