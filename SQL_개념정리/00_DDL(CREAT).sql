...
/*
테스트를 진행하거나 컬럼에 대한 설정을 변경한 후 특정한 내용을 확인하길 원한다면 테이블을 복제해서 사용할 수 있음(사본)

사본 만들기를 진행하는 방법
CREAT TABLE 새로운테이블이름 AS SELECT * FROM 복사할테이블이름;

특정한 컬럼만 선택해서 사본만들기
CREAT TAGER 새로운테이블 이름 AS SELECT 컬럼명1,컬럼명2, ... FROM 복사할테이블이름;

*/

CREATE TABLE EMPOLYEE2 AS SELECT * FROM EMPLOYEE;
