...
-- ALTER(바꾸다,수정하다)
-- 수정가능한 것 컬럼(추가/설정/삭제) 제약조건(추가/삭제) 이름변경(테이블,컬럼,제약조건)

--[작성법] 
-- 테이블을 수정하는 경우
-- ALTER TABER 테이블명 ADD / MODIFY / DROP 수정할 내용; 
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_COPY_UNI UNIQUE(DEPT_TITLE); --유니크 뒤에 가로열고 DEPT_TITLE 컬럼 추가
...
-- 삭제할 테이블 TB1 생성 
CREATE TABLE TB1(TBL_PK NUMBER PRIMARY KEY , TBL_COL NUMBER);

CREATE TABLE TB2(TB2_PK NUMBER PRIMARY KEY , TB2_COL NUMBER REFERENCES TB1); --TB1 테이블 PK값을 참조
