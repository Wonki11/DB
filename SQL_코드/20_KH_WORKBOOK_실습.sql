...
--SNACKS_SEQ 시퀀스 생성
-- 시퀀스명 SNACK_SEQ 시작값 1 증가값1 NOCACHE NOCYCLE 
CREATE SEQUENCE SNACKS_SEQ 
START WITH 1
INCREMENT BY 1
NOCACHE NOCYCLE;

SELECT * FROM SNACKS_SEQ;

--SNACKS_SEQ NEXTVAL 이용해서 INSERT해주기 
...
INSERT INTO SNACKS
VALUES(SNACKS_SEQ.NEXTVAL, '빼빼로' , 1100,2); 
...
SELECT * 
FROM COMPANY
WHERE COMPONY_ADDRESS LIKE '%강남구%'; 

-- 회사 ID가 1인 과자조회 SNACKS 
SELECT * 
FROM SNACKS 
WHERE COMPONY_ID = '1';

-- 회사ID가 1이고,그 회사의 과자들 조회 (JOIN) 
SELECT SNACK_NAME,COMPONY_ID 
FROM SNACKS
JOIN COMPANY USING(COMPONY_ID) 
WHERE COMPONY_ID = '1';